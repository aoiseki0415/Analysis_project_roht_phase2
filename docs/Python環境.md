...
ICLabelは、extended Infomax、平均基準、1–100 HzにフィルタしたEEGを前提として学習されています。本解析ではこの条件を満たすICA用コピーを作成します。瞬き成分は、ICLabelの `eye blink` 確率だけでなく、前頭部EEGから作る瞬きproxyとの相関、頭皮分布、瞬き様ピークを組み合わせた固定規則で完全自動選択します。個々のセッションで人が成分を選ばず、判定不能時は自動QC不合格として記録します。
