...
各IDのデータ保存後・前処理完了確定前に、このHTMLを最大3分だけ確認して瞬き成分除去精度を0～100点で評価します。完全ランダムな10秒区間を飛び飛びに確認し、BeforeでFp1・Fp2に同時に現れる約100 msの瞬き候補がAfterで両方から明瞭に消失・抑制した場合を1点、片側のみまたは部分的な抑制を0.5点、残存または悪化を0点として百分率化します。Fzは補助判定です。候補がなければ評価不能とし、推定値を作りません。評価値または評価不能理由をNotionの「ICA処理結果（ID別）」へ記録し、評価後は該当HTMLのSafariタブを閉じるボタンで必ず閉じます。
