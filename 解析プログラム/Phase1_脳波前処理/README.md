...
`--skip-local-data-output`は、利用者が明示的にローカル解析データを保存しないよう指示した比較・検証実行だけでNo1とNo2の両方に指定します。この場合、OneDriveの監査表、QC図、HTML、ログは作成しますが、`SandBox_ロート案件（データ）/解析に必要なデータたち/`のNo1マニフェスト、No2のHDF5、ICAモデル、メタデータは保存しません。保存コードは削除せず、フラグを外せば通常の完全保存に戻ります。このモードの実行はOneDrive QC用であり、最終解析データの保存完了とは扱いません。
