...
No2が記録全体への影響が明確な不良チャンネル候補を検出した場合は、フィルタ・トレンド除去後の0 µV中心波形をOneDriveへ保存し、利用者の決定まで停止します。承認後は `--approved-bad-channel <CH>`、残す場合は `--retained-bad-channel <CH>` を候補ごとに指定します。不良区間のICA学習用除外と、ICLabelの`eye blink >= 0.90`による成分除去は事前承認なしで実行します。
